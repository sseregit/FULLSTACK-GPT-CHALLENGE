...
def get_assistant(id):
  return next(
      (a for a in st.session_state["assistants"] if a["id"] == id),
      None
  )


def create_seesion_assistant(id):
  assistant = {
    "id": id,
    "assistant": {
      "messages": [{"message": "Please search for the keyword.", "role": "ai"}],
      "files": [],
    }
  }
...
if openai_api_key:
  try:
    ...
    assistant_api = create_assistant(openai_api_key)

    if assistant_api:
      assistant_id = assistant_api.id

      assistant = get_assistant(assistant_id)

      if assistant is None:
        assistant = create_seesion_assistant(assistant_id)

      for message in assistant["assistant"]["messages"]:
        with st.chat_message(message["role"]):
          st.markdown(message["message"])

      if user_input := st.chat_input():
        st.chat_message("user").write(user_input)
        assistant["assistant"]["messages"].append(
            {"message": user_input, "role": "user"})
                ...
                for followup_event in next_stream:
                  if isinstance(followup_event, ThreadRunRequiresAction):
                    ...
                    for action in run.required_action.submit_tool_outputs.tool_calls:
                      ...
                      outputs.append(
                          {
                            "output": functions_map[action.function.name](
                      ...
                      with st.chat_message("ai"):
                        ...
                        for followup_event in next_stream:
                          if isinstance(followup_event, ThreadMessageDelta):
                            delta_text = followup_event.data.delta.content[
                              0].text.value
                            full_text += delta_text
                            ai_placeholder.markdown(full_text)
                      assistant["assistant"]["messages"].append(
                          {"message": full_text, "role": "assistant"})
                      saved_files = st.session_state.get("saved_files", [])
                      for saved_file in saved_files:
                        try:
                          with open(saved_file, "rb") as file_handle:
                            safe_key = saved_file.replace(":", "-")
                            st.download_button(
                                label=f"{saved_file} 다운로드",
                                data=file_handle.read(),
                                file_name=saved_file,
                                key=f"download-{safe_key}",
                            )
